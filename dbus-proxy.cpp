...
handle_set_property_generic(G_GNUC_UNUSED GDBusConnection *connection,
  ...
  GVariant *result = g_dbus_connection_call_sync(
      ...
      g_variant_new("(ssv)", interface_name, property_name, value), NULL,
...
on_signal_received_catchall(GDBusConnection *connection G_GNUC_UNUSED,
  ...
  if (g_hash_table_contains(proxy_state->proxied_objects, object_path) ||
    ...
    gboolean success = g_dbus_connection_emit_signal(
        proxy_state->target_bus, NULL, object_path, interface_name, signal_name,
        parameters, &error);
...
static gboolean connect_to_buses() {
  ...
  proxy_state->source_bus =
      g_bus_get_sync(proxy_state->config.source_bus_type, NULL, &error);
  ...
  proxy_state->target_bus =
      g_bus_get_sync(proxy_state->config.target_bus_type, NULL, &error);
...
static void print_usage(const char *program_name) {
  ...
  g_print("Cross-bus D-Bus proxy that forwards method calls and signals "
          "between buses.\n\n");
  g_print("Options:\n");
  g_print("  --source-bus-name NAME     Source service bus name (example: "
          "org.freedesktop.NetworkManager)\n");
  g_print("  --source-object-path PATH  Source object path (example: "
          "/org/freedesktop/NetworkManager)\n");
  g_print("  --proxy-bus-name NAME      Proxy bus name (example: "
          "org.example.Proxy)\n");
  g_print("  --source-bus-type TYPE     Source bus type: system|session "
          "(default: system)\n");
